...
A `thread_id` is only unique **within one mailbox**, so every per-thread key and filter carries `integration_id` alongside `user_id`: the (non-unique) thread index on `tasks` is `(user_id, integration_id, thread_id)`, and `messages.list_by_thread_id_user_id` / `delete_by_thread` filter on all three. Without it two accounts of the same user sharing a `thread_id` would mix their tasks. This mirrors `messages`, whose unique key has always been `(integration_id, provider_id)`. `GET /messages/thread/{integration_id}/{thread_id}` takes the account in the path for the same reason.
